"""Same 4-panel layout step0_backward.py used before it grew the
learned-vs-exact-curve panels, but with a deliberately too large beta1 --
showing what happens once the single-step-is-Gaussian assumption clearly
breaks: q(x1) merges into one broad hump, and the exact reverse posterior
stops depending much on v at all, collapsing toward the unconditional
prior p(x0) (per the theoretical beta1->1 limit demonstrated
quantitatively in beta1_sweep.py).

Kept self-contained (not importing from step0_backward.py) since that
script's panels 3-4 now show a fixed-noise-level learned-vs-exact
comparison unrelated to this figure's large-beta1 point.
"""
...
def main(beta1=LARGE_BETA1):
    ...
    # --- panel 4: exact reverse posterior q(x0 | x1=v) ---
    ax = axes[1, 1]
    ymax4 = p0.max()
    for v, color in zip(example_vs, example_colors):
    ...
    setup_axis(ax, r"$q(x_0 \mid x_1=v)$: exact reverse posterior", 1.05 * ymax4, plot_range)
    ax.legend(fontsize=7, loc="upper right", ncol=2)

    fig.suptitle(rf"One DDPM step on 1D hue data ($\beta_1$={beta1:g}): forward noising and exact Bayesian reversal",
                 fontsize=13)
